...
def PCHA(X, noc, 
    ...
    try:
        if init=='furthest_sum':
            ...
            i = list(np.random.choice(np.array(I), noc, replace = False))
    except IndexError:
    ...
    while np.abs(dSSE) >= conv_crit * np.abs(SSE) and iter_ < maxiter and varexpl < var_threshold:
        ...
        if save_history:
            if iter_ % 10 == 0:
                hist[iter_] = varexpl, SSE, dSSE/np.abs(SSE), muC, mualpha, muS, XC, C, S
    ...
    if save_history:
        return XC, S, C, SSE, varexpl, hist
    else:
